...
## 21-08-2026 — Fase 10: Misura della copertura dei test e test dei casi limite
Introduzione della misura di copertura con pytest-cov e coverage, fissati in requirements.txt per riproducibilità in integrazione continua. Configurazione in pyproject.toml: la sezione [tool.coverage.run] limita la misura al pacchetto winery_adventures, la sezione [tool.coverage.report] attiva il dettaglio delle righe scoperte ed esclude dalla metrica le funzioni compilate da Numba, riconosciute dal decoratore @njit. Motivazione dell'esclusione: coverage.py osserva il bytecode Python, mentre il corpo di pairwise_stress_function esegue in codice nativo compilato e risulta perciò non strumentabile; la sua esclusione evita un falso negativo strutturale nella metrica, mentre la correttezza resta garantita dai test funzionali. Scartata l'alternativa della disabilitazione del JIT in fase di misura, che produrrebbe numeri reali ma farebbe fallire il test di verifica del Dispatcher Numba, e quella del pragma in linea, che allungherebbe oltre il limite una firma già estesa e modificherebbe un modulo già chiuso. Aggiunta di tests/unit/test_edge_cases.py, primo file di test proprio distinto dalle specifiche fornite, con la verifica del ramo di guardia della formula sul caso di insieme vuoto di rilevazioni, in precedenza privo di collaudo. Esito sulla macchina di lavoro: 18 test superati, copertura totale del codice strumentabile pari al 100%. File toccati: pyproject.toml, requirements.txt, tests/unit/test_edge_cases.py, DIARIO.md. In sospeso: integrazione della misura di copertura nel workflow di integrazione continua.

## 21-08-2026 — Fase 10: Integrazione della copertura nell'integrazione continua e chiusura fase
Estensione del workflow .github/workflows/ci.yml con la misura della copertura nello step dei test: il comando passa da pytest -v a pytest -v --cov --cov-fail-under=100. Il flag --cov attiva la misura, mentre la configurazione di cosa misurare e come riportarlo resta centralizzata in pyproject.toml, così il comportamento coincide tra ambiente locale e integrazione continua e il report compare nel log di ogni esecuzione. Il flag --cov-fail-under=100 introduce un cancello di qualità: la copertura sotto la soglia fa terminare l'esecuzione con errore e segnala la regressione sulle pull request. Motivazione della soglia al 100%: il valore è genuino e mantenibile poiché dalla metrica è esclusa soltanto la funzione compilata da Numba, non osservabile, mentre il resto del codice è integralmente coperto; il cancello impedisce l'introduzione futura di codice privo di test. La modifica presuppone la presenza di pytest-cov tra le dipendenze fissate, introdotta nella tappa precedente della fase. Con questo passo la Fase 10 raggiunge i propri obiettivi: misura della copertura, individuazione del codice non coperto, aggiunta del test mirato sul caso limite e integrazione della misura nell'integrazione continua. File toccati: .github/workflows/ci.yml, DIARIO.md. Fase 10 conclusa.